...
|---|---|---|---|---|---|---|---|
| 3 | Homepage rewrite to "Your AI workforce, off the shelf" — new hero, 3-col feature section, Hermes/OpenClaw trust stack, Browse-agents CTA. Legacy skills surface demoted to footer link. | (pending) | H-005 | Homepage → `/agents` click-through rate (starts measuring once H-006 lands a real `/agents` page); secondary: homepage → `/skills` referrals should drop sharply. | 0 measurable clicks (prior page sent supply-side signups to `/sign-up`). | Tick 8 (5 ticks post-ship, per kill rule) | — |
